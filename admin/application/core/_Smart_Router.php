...
class Smart_Router extends CI_Router {
	...
	if(!empty($directory))
	{	
		if(isset($session['id']) && !empty($session['id']) && $session['level']==2 && $class!='login')
		{
			$directory .= 'management/';
		}
		if(isset($session['id']) && !empty($session['id']) && $session['level']==5 && $class!='login')
		{
			$directory .= 'pengantar/';
		}
		if(isset($session['id']) && !empty($session['id']) && $session['level']==3 && $class!='login')
		{
			$directory .= 'storemanager/';
		}
		elseif((!isset($session['id']) || empty($session['id'])) &&  $class!='login')
		{
			$class = 'login';
		}
	}
