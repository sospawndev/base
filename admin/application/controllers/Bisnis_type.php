...
class Bisnis_type extends Smart_Controller
{
	private $_users;
	...
	public function index()
	{
		...
		//exit;
		$in['use_hedaer'] = true;
		$in['title'] = 'What kind of business';
		$in['desc'] = 'you can manage your What kind of business here';
		$in['bread'][''] = 'Configuration';
		$in['bread'][site_url('manager/bisnis_type')] = 'What kind of business';
		$in['tpl'] = 'bisnis_type/main';
	...
	public function getlist()
	{
		if($this->input->is_ajax_request())
		{
			...
			$ssp = $this->ssp;
			$primaryKey = 'm_bisnis_type.id';
			$columns    = array(
			array('db'=>'m_bisnis_type.id','dt'=>0,'alias'=>'ids','formatter'=>function($d,$row){
				return '<input type="checkbox" class="chk-item" value="'.$d.'">';
			}),
			
			 
			array('db'=>'m_bisnis_type.name','dt'=>1,'alias'=>'bisnis_type','formatter'=>function($d,$row){
				return $d;
			}), 
			array('db'=>"m_bisnis_type.displays",'dt'=>2,'alias'=>'displays','formatter'=>function($d,$row){
				 if($d==1)
				 {
			...
			array('db'=>'m_bisnis_type.id','dt'=>3,'alias'=>'id','formatter'=>function($d,$row){
	...
	public function delete_logo()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			$this->db->update('bisnis_type',array('image'=>''));
	...
	public function save()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			if(empty($in['id']))
			{
				...
				$in['created_on'] = $time;
				$in['updated_on'] = $time;
				$this->db->insert('bisnis_type',$in);
			}
			else
			{
				$this->db->where('id',$in['id']);
				$old = $this->db->get('bisnis_type');
				if($old->num_rows()==1)
				{
					$arr = $old->row_array();
	...
	public function displays()
	{
		if($this->input->post() && $this->input->is_ajax_request())
		{
			...
			$this->db->trans_begin();	
			$this->db->where('id',$in['id'])
			->update('bisnis_type',array("displays"=>$in['displays']));
