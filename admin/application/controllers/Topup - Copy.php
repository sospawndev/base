...
class Topup extends Smart_Controller
{
	...
	public function getlist()
	{
		if($this->input->is_ajax_request())
		{
			...
			array('db'=>"m_topup.promo_code",'dt'=>5,'alias'=>'promo_code','formatter'=>function($d,$row){
				return $d;
			}), 
			array('db'=>"m_topup.value_total",'dt'=>6,'alias'=>'value_total','formatter'=>function($d,$row){
				return number_format($d,2) ;
			}), 
			array('db'=>"m_topup.total",'dt'=>7,'alias'=>'total','formatter'=>function($d,$row){
			...
			array('db'=>"m_topup.received_address",'dt'=>8,'alias'=>'received_address'),
			array('db'=>"m_topup.tx_hash",'dt'=>9,'alias'=>'tx_hash'),
			array('db'=>"m_topup.status",'dt'=>10,'alias'=>'status','formatter'=>function($d,$row){
			 ...
			 array('db'=>'m_topup.id','dt'=>11,'alias'=>'id','formatter'=>function($d,$row){
				if($row->status!=1)
				{
					return ' 
			 ...
			 array('db'=>"m_topup.id",'dt'=>12,'alias'=>'ids'),
			 array('db'=>"m_topup.promo_value",'dt'=>13,'alias'=>'promo_value'),
			);
			$table = 'm_topup inner join m_customer on (m_customer.id = m_topup.id_customer)';
			$whereResult = NULL;
			$whereAll = '1=1';
