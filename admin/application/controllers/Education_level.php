...
class Education_level extends Smart_Controller
{
	private $_users;
	...
	public function index()
	{
		...
		//exit;
		$in['use_hedaer'] = true;
		$in['title'] = 'Education level';
		$in['desc'] = 'you can manage your Education_level here';
		$in['bread'][''] = 'Configuration';
		$in['bread'][site_url('manager/education_level')] = 'Bank';
		$in['tpl'] = 'education_level/main';
	...
	public function getlist()
	{
		if($this->input->is_ajax_request())
		{
			...
			$ssp = $this->ssp;
			$primaryKey = 'm_education_level.id';
			$columns    = array(
			array('db'=>'m_education_level.id','dt'=>0,'alias'=>'ids','formatter'=>function($d,$row){
				return '<input type="checkbox" class="chk-item" value="'.$d.'">';
			}),
			
			 
			array('db'=>'m_education_level.name','dt'=>1,'alias'=>'education_level','formatter'=>function($d,$row){
				return $d;
			}), 
			 
			array('db'=>"m_education_level.displays",'dt'=>2,'alias'=>'displays','formatter'=>function($d,$row){
				 if($d==1)
				 {
			...
			array('db'=>'m_education_level.id','dt'=>3,'alias'=>'id','formatter'=>function($d,$row){
	...
	public function delete_logo()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			$this->db->where('id',$id);
			$rs = $this->db->get('education_level');
			if($rs->nus_rows()!=1)
			{
			...
			$this->db->update('education_level',array('image'=>''));
	...
	public function match()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			$data = $this->db->get('education_level');
			if($data->num_rows()==1)
			{
	...
	public function save()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			if(empty($in['id']))
			{
				...
				$in['created_on'] = $time;
				$in['updated_on'] = $time;
				$this->db->insert('education_level',$in);
			}
			else
			{
				$this->db->where('id',$in['id']);
				$old = $this->db->get('education_level');
				if($old->num_rows()==1)
				{
					$arr = $old->row_array();
					...
					$in['updated_on'] = $time;
					$this->db->where('id',$in['id']);
					$this->db->update('education_level',$in);
				}
				else
				{
	...
	public function displays()
	{
		if($this->input->post() && $this->input->is_ajax_request())
		{
			...
			$this->db->trans_begin();	
			$this->db->where('id',$in['id'])
			->update('education_level',array("displays"=>$in['displays']));
