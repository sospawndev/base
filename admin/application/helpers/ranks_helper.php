...
function level_earn_from_umbrella($userid = "",$vals = 0)
{
	...
	if(is_array($ref_arr))
	{
		for($i=0;$i<count($ref_arr);$i++)
		{
			...
			$invest_coin += $ref_arr[$i]['buy_coin'];
			$ref_arr[$i]['direct'] = false;
			if($userid==$ref_arr[$i]['refferal'])
			{
				$ref_arr[$i]['direct'] = true;
			}
			$ref_arr[$i]['level_users_n'] = small_level($ref_arr[$i]['id']);
			$v_num[$ref_arr[$i]['level_users_n']][] = $ref_arr[$i]['level_users_n'];
			$a = array();
			$a['direct'] = $ref_arr[$i]['direct'];
			$a['level'] = $ref_arr[$i]['level_users_n'];
			
			$v_check[$ref_arr[$i]['level_users_n']][] = $a;
		}
	}
	$top_check = 0;
	$top_id = 0;
	...
	for($i=0;$i<count($arr);$i++)
	{
		...
		}else
		{
			...
			if(isset($uu['id']))
			{
				...
				/*
				if($arr[$i]['under_umbrealla']==-1)
				{
					$vcheck = 0;
					$levelo = isset($v_check[$uu['id']])?$v_check[$uu['id']]:array();
					
					if(count($levelo)>0)
					{
						for($k=0;$k<count($levelo);$k++)
						{
							if(isset($levelo[$k]['direct']))
							{
								if($levelo[$k]['direct']==true)
								{
									$vcheck +=1;
								}
...
function top_earn_from_umbrella($userid = "",$vals = 0)
{
	$CI =& get_instance();
	...
	$arr = $CI->db->where("level_from_name>0")
				  ->where("min_invest <= '".$invest_coin."'") 
				  ->where("under_umbrealla = -1")
				  ->order_by("level_from desc")
				  //->limit(1)
				  ->get('v_level_earn')->result_array();
	for($i=0;$i<count($arr);$i++)
	{
...
function small_level($id_users = "")
{
	...
	$arr = $CI->db
				 ...
				 ->limit(1)
				 ->get('v_level_earn')->row_array();
	return isset($arr['id'])?$arr['id']:bottom_level();				 	
}
function my_level($id_users = "")
{
	...
	//{
		...
		//manual
		$ref_count = ref_count_buy($id_users); 
		$top = top_earn_from_umbrella($id_users,$ref_count); 
		$up = level_earn_from_umbrella($id_users,$ref_count);
	 
		if($top>0)
		{
			$arr = $CI->db->where('id',$top)->get('v_level_earn')->row_array();
		}
		else if($up>0)
		{
