                          ...
                          <hr/>
                              ...
                              <div class="col-12">
                                  <span class="text-center">
                                                 Balance PG:  <strong class="alert alert-info balancings">Uknown</strong>
                                  </span> 
                              </div>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <div class="form-group  hashed" style="display:none !important;">
...
<div id="modalinfos" class="modal fade" role="dialog">
  <div class="modal-dialog">
	<form action="" method="post" id="frm-infos">
    <!-- Modal content-->
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Form Info Linkque</h4>
      </div>
      <div class="modal-body">
        <!-- -->
        	<div id="wdinfos">
      ...
      <div class="modal-footer">
        ...
        <input type="hidden" name="<?=$this->security->get_csrf_token_name()?>" value="<?=$this->security->get_csrf_hash();?>" class="smart-token">
        -->
      	<button type="submit" class="btn btn-primary" id="btn-wdinfo">Update</button>
        <button type="button" class="btn btn-default close" data-dismiss="modal">Close</button>
      </div>
    </div>
	</form>
...
$(document).ready(function(){
	...
	//
	$(".js-dataTable-full").on('click','.btn-infos',function(){
		var ids = $(this).data('ref');
		var partner_reff = $(this).data('partner_reff');
		if(ids!="" && partner_reff!="")
		{
			 var req = get('<?=site_url('withdraw/report_get')?>',{id:ids,"partner_reff":partner_reff,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								$("#modalinfos #id").val(ids);
								console.log(out.data.data);
								$("#modalinfos #status").val(out.data.data.status);
								$("#modalinfos #wdinfos").html(out.data.temps);
								if(out.data.data.status==2)
								{
									//if(out.data.data.status_paid=="paid")
									$("#modalinfos #btn-wdinfo").show();
								}else
								{
									$("#modalinfos #btn-wdinfo").hide();
									if(out.check.status==3 && (out.data.data.status_paid=="paid"))
									{
										//if(out.data.data.status_paid=="paid")
										$("#modalinfos #btn-wdinfo").show();
									}
	...
	$("#frm-infos").validate({
		...
		submitHandler:function(){
			var req = post('<?=site_url('withdraw/status_check')?>',$("#frm-infos").serialize());
			req.done(function(out){
				if(!out.error)
				{
					table_ajax.api().ajax.reload();
...
function get_balancing()
{
	var req = get('<?=site_url('withdraw/balance_get')?>',{id:1,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								 $(".balancings").text("Rp. "+ out.data.balance);
							}
							else
							{
