    	...
    	<div class="card">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    ...
                    <input type="text" class="form-control required" name="atas_nama" id="atas_nama"> 
                </div>
                <div class="form-group">
                	<label>Alamat</label>
...
var BaseTableDatatables = function() {
    ...
    var initDataTableFull = function() {
        table_ajax = $('.js-dataTable-full').dataTable({
			...
			"serverSide": true, 
			"ajax": {
					url :"<?=site_url('bank_transfer/getlist')?>",
					type : "GET",
					data : function(d){
...
$(document).ready(function(){
	...
	$("#frm-object").validate({
		...
		submitHandler:function(){
			...
			var req = postFile('<?=site_url('bank_transfer/save')?>',formdata);
	...
	$(".js-dataTable-full").on('click','.btn-edit-sites',function(){
		...
		if(ids!="")
		{
			var req = post('<?=site_url('bank_transfer/match')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
			req.done(function(out){
				if(!out.error)
				{
					$("#id").val(out.data.id);
					$("#no_rekening").val(out.data.no_rekening);
					$("#atas_nama").val(out.data.atas_nama);
					$("#address").val(out.data.address);
					$("#img-container").val(out.data.qrcodes);
					$("#id_bank").val(out.data.id_bank).trigger('change');
				}
				else
				{
					smart_error_box(out.message,'#frm-object .block-content');
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('bank_transfer/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('bank-transfer/displays')?>',{id:ids,"displays":checks,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
