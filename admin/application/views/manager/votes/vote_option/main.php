   ...
   <div class="row">
            <div class="col-md-12">
                    <div class="card">
                        ...
                        List <?=$title?>
                        <small><b><?=$vote['titles']?></b></small>
                         <span class="pull-right">
                              
                          	 
                             <a   href="<?=site_url('votes/vote-option/add/'.$param_id)?>" class="btn btn-primary btn-sm btn-mini btn-xs pull-right" type="button" data-toggle="tooltip" title="" data-original-title="add"><i class="fa fa-plus"></i></a>
                             <a   href="<?=site_url('votes/vote')?>" class="btn btn-warning btn-sm btn-mini btn-xs pull-right" type="button" data-toggle="tooltip" title="" data-original-title="add"><i class="fa fa-arrow-left"></i> Back</a>
                         </span>  
                      </div>
                      <div class="card-body">
...
<script>
var table_ajax;
var curls = "<?=site_url('votes/vote-option/getlist/'.$param_id)?>?baru=1";
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('votes/vote-option/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
		...
		if(ids!=""  )
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('votes/vote-option/defaults')?>',{id:ids,'displays':displays ,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
