	...
	<div class="col-md-12">
   	   ...
   	   <div class="card">
       		<div class="card-header">
                   Form <?=$title?>  <small><b><?=$vote['titles']?></b></small>
            </div>
            <div class="card-body">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                <label>Name</label>
                                                <input type="text" class="form-control required" name="name" id="name" placeholder="name" value="<?=isset($data['name'])?$data['name']:""?>"   /> 
                                            </div> 
                                       </div>     
                                   </div>     
                                ...
                                <br/>
                                  ...
                                  <div class="form-group">
                                    ...
                                    <input type="hidden" name="id_vote" value="<?=isset($data['id_vote'])?$data['id_vote']:$param_id?>" id="id_vote" />
                                    <!-- 
...
$(document).ready(function(){
	$("#frm-object").validate({
		...
		submitHandler:function(){
			...
			var req = postFile('<?=site_url("votes/vote-option/save")?>',data);
			req.done(function(out){
				if(!out.error)
				{
					smart_success_box(out.message,'#frm-object .card-body');
					document.location.href="<?=site_url('votes/vote-option/index/'.$param_id)?>";
				}
				else
				{
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('votes/vote-option/delete-image')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
