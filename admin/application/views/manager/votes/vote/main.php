                    ...
                    <div class="card">
                        ...
                        List <?=$title?>
                         <span class="pull-right">
                          	 ...
                          	 <a   href="<?=site_url('votes/vote/add')?>" class="btn btn-sm btn-mini btn-xs pull-right" type="button" data-toggle="tooltip" title="" data-original-title="add"><i class="fa fa-plus"></i></a>
                         </span>  
                      </div>
                      <div class="card-body">
                          ...
                          <p>
                          	<b><i>Note: click arrow right to create option</i></b>
                          </p>  
                      </div>
...
<script>
var table_ajax;
var curls = "<?=site_url('votes/vote/getlist')?>?baru=1";
var BaseTableDatatables = function() {
    ...
    var initDataTableFull = function() {
        table_ajax = $('.js-dataTable-full').dataTable({
			...
			{ orderable: false, searchable:false, targets: [ 5 ],className:'text-center' },
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('votes/vote/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
		...
		if(ids!=""  )
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('votes/vote/defaults')?>',{id:ids,'displays':displays ,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
