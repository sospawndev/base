...
<script>
var table_ajax;
var curls = "<?=site_url('buy/getlist')?>";
...
$(document).ready(function(){
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('buy/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
	...
	$("#frm-status").validate({
		...
		submitHandler:function(){
			var req = post('<?=site_url('buy/status')?>',$("#frm-status").serialize());
			req.done(function(out){
				if(!out.error)
				{
					table_ajax.api().ajax.reload();
	...
	$("#frm-tx").validate({
		...
		submitHandler:function(){
			var req = post('<?=site_url('buy/updatetx')?>',$("#frm-tx").serialize());
			req.done(function(out){
				if(!out.error)
				{
					table_ajax.api().ajax.reload();
