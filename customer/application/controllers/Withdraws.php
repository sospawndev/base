...
class Withdraws extends Smart_Controller {
	...
	public function save_tf()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			{
				json(array('error'=>true,'message'=>'Cannot Be Process, Must Wait Until Tomorrow','security'=>token()));
				return;
			}
			...
			$balance_close = floatval(setting("balance_close"));  
			...
			{
				json(array('error'=>false,'message'=>'Process Failed ','security'=>token()));
				return;
			}
			 
				if($resumeAccount->balance<=$balance_close)
				{
					$this->db->trans_begin();
					$this->db->where('id',1)->update('setting',array("withdraw_page"=>0));
					$this->db->trans_commit();
					json(array('error'=>false,"autoclose"=>true,'message'=>'autooclose','security'=>token()));
					return;
				}
			$trx = $linkqu->transfer();
			$waktu = time();
			...
			if(!isset($arr->status))
			{
				json(array('error'=>true,'message'=>'Proccess Faileds','security'=>token()));
				return;
			}
			if(!isset($arr->accountname))
			{
				json(array('error'=>true,'message'=>'Proccess Bank Account Name Faileds, Choose another Bank or Change number Rek','security'=>token()));
				return;
			}
			if(!isset($arr->inquiry_reff))
			{
				json(array('error'=>true,'message'=>'Proccess inquiry reff Failed With Bank Account Name is '.$arr->accountname,'security'=>token()));
				return;
			}
			...
			$inquiry_reff = $arr->inquiry_reff;
			$accountname = $arr->accountname;
			if($atas_nama != $accountname)
			{
				
				json(array('error'=>true,'message'=>'Atas Nama Anda diprofile Salah, Atas Nama Tertera di Akun Bank Langsung '.$accountname,'security'=>token()));
				return;
			}
			$check_atas_nama = $this->db
					 ->where("id<>".user_info('id')."") 
			...
			*/
			...
			$v['atas_nama'] = $accountname; //user_balance('atas_nama');
