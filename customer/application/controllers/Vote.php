...
class Vote extends Smart_Controller {
	...
	public function save()
	{
		if($this->input->is_ajax_request() && $this->input->post())
		{
			...
			$r['txhash'] = $smartcontract->send(user_balance('wallet_address'),$reward);
			//
			...
			else
			{
				...
				$curls = site_url('vote/views/'.$id);
				json(array('error'=>false,'message'=>'Proccess Done',"id"=>$id,'url'=>$curls,'security'=>token()));
				return;
			}
			json(array('error'=>true,'message'=>'Proccess Failed','security'=>token()));
			return;
	...
	public function views($params)
	{
		if(!empty($params))
		{
			//$rec = $this->db->where('id',$params)->get('v_vote');
			$rec = $this->db
					->select("m_v_vote.*,m_vote_reward.reward,m_vote_reward.network_url,m_vote_reward.txhash")
					->where('m_vote_reward.id',$params)
					->join("m_v_vote","m_v_vote.id=m_vote_reward.id_vote","inner")
					->get('vote_reward');
			if($rec->num_rows()==1)
			{
				$data = $rec->row_array();	
