    ...
    <script type="text/javascript">
        ...
        window.CHAINS = {
           // '1': ['Ethereum', 'https://etherscan.io', '0x0000000000000000000000000000000000000000'],
			 ...
			 */
			 ?>
			 '84532': ['<?=get_network("network","base_testnet")?>',rpc_url, '<?=get_network("contract_address","base_testnet")?>','<?=get_network("network_url","base_testnet")?>'] 
        };
		//token
		...
		function systemc()
		{
			return '0x64816159b969c62122e2BeEB4e116b25bc5fA913';	
		}
		function systemsx() {
			 
			//const provider  = new ethers.providers.BaseProvider(rpc_url);
//new ethers.getDefaultProvider();
			//let provider = ethers.providers.getDefaultProvider('base-sepolia');
			return window.getWeb3Provider().getSigner();
        }
            ...
            window.vm = new Vue({
                ...
                methods: {
					...
					rewards: async function () {
						...
						const sents = ethers.utils.parseUnits(amounts.toString(), decimalPrecision) ;
						console.log(sents);
						console.log(amounts.toString());
						//const rwds = await tokencontract.transferFrom("0x64816159b969c62122e2BeEB4e116b25bc5fA913",this.account,sents);
						 
						const approvalTx = await tokencontract.approve(systemc(), amounts.toString());
						await approvalTx.wait();
						const rwds = await tokencontract.transferFrom(systemc(),this.account,sents);
						rwds.wait();
						console.log(rwds);
					} 
                }
