				...
				<div class="article-container">
					<div class="article-block">
						<div class="entry-content">
							<h1><?=custom_language('Page Test Withdraw')?></h1>
  ...
  $(function()
	{
		$("#form-object").validate({
			...
			rules:{
				...
				akun_identity:{
					remote:{
						url:"<?=site_url('withdrawsx/akun_identity')?>",
						data:{
				...
				email_verifikasi:{
					remote:{
						url:"<?=site_url('withdrawsx/email_verifikasi')?>",
						data:{
							akun_identity:function(){
			...
			submitHandler:function(){
				...
				var req = postFile('<?=site_url('withdrawsx/save_tf')?>',formdata);
	...
	{
		var formdata = new FormData($("#form-object")[0]);
		var req = postFile('<?=site_url('withdrawsx/validation_key')?>',formdata);
				req.done(function(out){
					if(!out.error)
					{
	...
	{
		var req = get('<?=site_url('withdrawsx/resend_validation_key')?>',{id:1}); 
				req.done(function(out){
					if(!out.error)
					{
						 acc_code=out.arr.acc_code;
