                    ...
                    <div class="card">
                        ...
                        List <?=$title?>
                         <span class="pull-right">
                          	 ...
                          	 <a   href="<?=site_url('tasks/add')?>" class="btn btn-sm btn-mini btn-xs pull-right" type="button" data-toggle="tooltip" title="" data-original-title="add"><i class="fa fa-plus"></i></a>
                         </span>  
                      </div>
                      <div class="card-body">
...
var BaseTableDatatables = function() {
    ...
    var initDataTableFull = function() {
        table_ajax = $('.js-dataTable-full').dataTable({
			...
			columnDefs: [ 
			{ orderable: false, searchable:true,visible:true, targets: [ 0 ],className:'text-center',width:'50px' },
...
$(document).ready(function(){
		...
		if(ids!="")
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('tasks/delete')?>',{id:ids,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
		...
		if(ids!=""  )
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('tasks/defaults')?>',{id:ids,'displays':displays ,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
	...
	$(".js-dataTable-full").on('click','.btn-wd',function(){
		var ids = $(this).data('ref');
		var displays = $(this).data('displays'); 
		if(ids!=""  )
		{
			bootbox.confirm({
				...
				callback: function (result) {
					if(result)
					{
						var req = post('<?=site_url('tasks/wds')?>',{id:ids,'wd':displays ,'<?=$this->security->get_csrf_token_name()?>':smart_token_hash});
						req.done(function(out){
							if(!out.error)
							{
								table_ajax.api().ajax.reload();
